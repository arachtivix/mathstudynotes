...
This folder contains code generated by Amazon Q and lightly edited by myself.  The scripts do some super simple things in blender.  It runs using the pypi distribution of blender so referencing or attempting to utilize elements of the blender ui in these scripts is rather useless -- this includes causing things to happen in the editor that display only in the blender viewport.  Scripts should attempt to reuse code, especially code in files without the word "demo" in them.  The "demo" scripts can be used as reference though.

Here's the `pip list` for the environment running this folder right now:
Package            Version
------------------ ---------
bpy                4.4.0
certifi            2025.1.31
